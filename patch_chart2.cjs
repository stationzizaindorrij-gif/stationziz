          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                ...
                <YAxis stroke="#94a3b8" fontSize={11} tickLine={false} tickFormatter={(value) => Math.round(Number(value)).toString()} width={45} />
...
if (code.includes('Volume (Litres) cumulé')) {
  code = code.replace(target, replace);
} else {
  // If my previous patch didn't apply or was reverted, replace the original one:
  const origTarget = `        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-bold text-slate-900 font-display">Courbe d'activité</h3>
              <p className="text-xs text-slate-400">Volume (Litres) et chiffre d'affaires cumulés</p>
  ...
  code = code.replace(origTarget, replace);
}
