...
import { 
  ...
  Printer, Info, BarChart3, Filter, CheckCircle2, Layers, Calendar,
...
export default function StockCalc({ store }: StockCalcProps) {
  ...
  const pumpSimulationResults = useMemo(() => {
    ...
    return pumpMetaList.map(item => {
      ...
      const hasInput = item.entries > 0 || item.exits > 0;
      const ecartLitres = hasInput ? (item.quantiteConsommee - item.achatsReels) : 0;
      const ecartFinancier = hasInput ? (caReel - item.costTotal) : 0;
  ...
  const totalsSimulation = useMemo(() => {
    ...
    const ecartLitresTotal = selectedSimulationResults.reduce((acc, r) => acc + r.ecartLitres, 0);
  ...
  return (
    <div className="space-y-6 pb-12" id="calcul-stock-view">
      ...
      {/* Selector of Analysis Period - Simplified Custom Date Range */}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-semibold text-slate-500">Choisissez la plage de dates pour filtrer les calculs et rapports</p>
            </div>
          </div>
          <span className="self-start sm:self-auto text-xs font-bold px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-xl border border-indigo-100 shadow-2xs">
            {periodLabel}
          </span>
        </div>

        {/* Large Prominent Date Range Inputs */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
          <div className="flex flex-col gap-1.5">
            ...
            <input 
              ...
              className="px-4 py-3 border-2 border-slate-200 rounded-xl text-sm font-bold text-slate-800 bg-slate-50/50 hover:bg-white focus:bg-white focus:border-indigo-600 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all cursor-pointer shadow-xs"
            />
          </div>
              ...
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="bg-slate-100/70 border-b border-slate-200 text-xs font-bold text-slate-600 uppercase tracking-wider">
                    ...
                    <th className="py-2.5 px-3 text-right whitespace-nowrap">Écart (L)</th>
                  </tr>
                </thead>
                  ...
                  {selectedSimulationResults.length === 0 ? (
                    ...
                    selectedSimulationResults.map((item) => (
                      <tr key={item.pumpKey} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="py-2.5 px-3 text-right font-mono whitespace-nowrap">
                          {(() => {
                            const ecart = item.ecartLitres;
                            if (ecart > 0) return <span className="font-bold text-emerald-600 block text-sm">+{formatLiters(ecart)}</span>;
                            if (ecart < 0) return <span className="font-bold text-rose-600 block text-sm">{formatLiters(ecart)}</span>;
                            return <span className="font-bold text-slate-400 block text-sm">{formatLiters(0)}</span>;
                          })()}
                        </td>
                      </tr>
                    ))
                ...
                <tfoot>
                  <tr className="bg-slate-100/90 border-t-2 border-slate-300 font-black text-slate-900 text-sm">
                    ...
                    <td className="py-2.5 px-3 text-right font-mono whitespace-nowrap">
                      {(() => {
                        const ecartTotal = totalsSimulation.ecartLitresTotal;
                        if (ecartTotal > 0) return <span className="font-black text-emerald-600 block text-sm">+{formatLiters(ecartTotal)}</span>;
                        if (ecartTotal < 0) return <span className="font-black text-rose-600 block text-sm">{formatLiters(ecartTotal)}</span>;
                        return <span className="font-black text-slate-400 block text-sm">{formatLiters(0)}</span>;
                      })()}
                    </td>
                  </tr>
                </tfoot>
